...
# --- パーサー (Parser) の定義 ---
def p_person_names(p):
    '''person_names : person_name_block
                   | person_names person_name_block
                   | empty'''
    if len(p) == 2:
        if p[1] is None:
...
def p_person_name_block(p):
    '''person_name_block : IDENTIFIER EQUALS LBRACE block_content RBRACE'''
    p[0] = {
        'tag': p[1],
        'content': p[4]
    }
...
class PersonNameParser:
    ...
    def parse(self):
        ...
        try:
            ...
            names_by_tag = {}
            for block in result:
                if 'tag' in block and 'content' in block:
                    tag = block['tag']
                    ...
                    # 男性名の処理
                    if 'male' in content and 'names' in content['male']:
                        names_by_tag[tag]['male_name'] = content['male']['names']

                    # 女性名の処理
                    if 'female' in content and 'names' in content['female']:
                        names_by_tag[tag]['female_name'] = content['female']['names']

                    # 姓の処理
                    if 'surnames' in content:
                        names_by_tag[tag]['surname'] = content['surnames']

                    # コールサインの処理
                    if 'callsigns' in content:
                        names_by_tag[tag]['callsign'] = content['callsigns']
